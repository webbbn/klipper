...
uint32_t SystemCoreClock = FREQ_MAIN;

// Enable a peripheral clock and power to that peripheral
void
enable_pclock(uint32_t pclk_id, uint32_t pm_id)
{
    MCLK->APBCMASK.reg |= pm_id;
    GCLK->PCHCTRL[pclk_id].reg = GCLK_PCHCTRL_GEN(0) | GCLK_PCHCTRL_CHEN;
    while (0 == (GCLK->PCHCTRL[pclk_id].reg & GCLK_PCHCTRL_CHEN));
}
...
{
  // set CPU to 48MHz and adjust flash wait states accordingly: 3 wait states for 3.3V; 2 wait state for 5V
  NVMCTRL->CTRLB.reg = NVMCTRL_CTRLB_READMODE_NO_MISS_PENALTY | NVMCTRL_CTRLB_SLEEPPRM_WAKEUPINSTANT | NVMCTRL_CTRLB_MANW | NVMCTRL_CTRLB_RWS(2);
